...
class APNs {
    constructor(db, env = {}) {
        const requiredEnv = (name) => {
            ...
            if (!value) {
                throw new Error(`Missing required Bark APNs environment variable: ${name}`)
            }
            return value
        }
        ...
        const generateAuthToken = async () => {
            const TOKEN_KEY = requiredEnv('BARK_APNS_PRIVATE_KEY')
            ...
            const TEAM_ID = requiredEnv('BARK_APNS_TEAM_ID')
            const AUTH_KEY_ID = requiredEnv('BARK_APNS_KEY_ID')
            // Generate the JWT token
            const JWT_ISSUE_TIME = util.getTimestamp()
            const JWT_HEADER = base64Url(btoa(JSON.stringify({ alg: 'ES256', kid: AUTH_KEY_ID })))
            const JWT_CLAIMS = base64Url(btoa(JSON.stringify({ iss: TEAM_ID, iat: JWT_ISSUE_TIME })))
            ...
            const JWT_SIGNED_HEADER_CLAIMS = base64Url(btoa(String.fromCharCode(...signatureArray)))
        ...
        this.push = async (deviceToken, headers, aps) => {
            const TOPIC = requiredEnv('BARK_APNS_TOPIC')
            const APNS_HOST_NAME = env.BARK_APNS_HOST_NAME || 'api.push.apple.com'
